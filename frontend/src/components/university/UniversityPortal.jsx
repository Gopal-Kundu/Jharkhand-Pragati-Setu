  ...
  const handleSubmitProposal = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        peopleImpacted: Number(proposalForm.peopleImpacted) || 0,
        industrySupportRequired: proposalForm.industrySupportRequired
      };

      const res = await universityApi.createProposal(payload);
      setIsProposalModalOpen(false);
      toast.success(res.message || 'Proposal sent to required industries');

      // Optimistic state update without page refresh
      const univId = myUniversity?._id || authUser?.university;
      if (univId) {
        setAllProblems(prev =>
          prev.map(p => {
            const matchId = (p._id || p.ticketId)?.toString();
            const targetId = problemId?.toString();
            if (matchId === targetId) {
              const currentGiven = Array.isArray(p.proposalGivenUniversity) ? p.proposalGivenUniversity : [];
              return {
                ...p,
                proposalGivenUniversity: [...currentGiven, univId]
              };
            }
            return p;
          })
        ...
        {activeTab === 'accepted' ? (
                      ...
                      <div className="relative h-44 w-full overflow-hidden bg-slate-100">
                        ...
                        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs">
                          ...
                          {prop.peopleImpacted && Number(prop.peopleImpacted) > 0 && (
                            <span className="bg-indigo-950/80 backdrop-blur-md text-indigo-300 px-2.5 py-1 rounded-xl font-bold border border-indigo-500/30">
                              👥 {Number(prop.peopleImpacted).toLocaleString()} Beneficiaries
                            </span>
                          )}
                        </div>
                      </div>
                    ) : (
                      <div className="relative h-32 w-full bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-950 p-4 flex flex-col justify-between">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="bg-emerald-950/80 backdrop-blur-md text-emerald-300 px-2.5 py-1 rounded-xl text-xs font-bold border border-emerald-500/30">
                            CSR Grant: ₹{(offer.fundingAmount || prop.estimatedBudget || 0).toLocaleString()}
                          </span>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {displayedProblems.map((item, idx) => {
              ...
              const imageUrl = item.evidence?.[0]?.url || item.evidenceUrl || '';
              const hasVideo = item.evidence?.some(e => e.type === 'video' || e.url?.endsWith('.mp4') || e.url?.includes('/video/'));

              // Check if current university has already made a proposal for this problem
              const currentUnivId = String(myUniversity._id || authUser?.university || '');
              const proposalGiven = Array.isArray(item.proposalGivenUniversity) ? item.proposalGivenUniversity : [];
              const hasProposalMade = 
                proposalGiven.some(uId => {
                  const resolvedId = typeof uId === 'object' && uId !== null ? String(uId._id || uId) : String(uId);
                  return Boolean(resolvedId && currentUnivId && resolvedId === currentUnivId);
                }) ||
                myProposals.some(prop => {
                  const propProbId = prop.problem?._id || prop.problem;
                  return Boolean(propProbId && (propProbId === item._id || propProbId === item.ticketId || prop.problemStatement === item.title));
                });
                ...
                >
                    ...
                    <div className="relative h-32 w-full bg-gradient-to-br from-slate-800 via-slate-900 to-emerald-950 p-4 flex flex-col justify-between">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5">
                          ...
                          {hasProposalMade && (
                            <span className="flex items-center space-x-1 text-[11px] font-bold text-purple-900 bg-purple-100/90 backdrop-blur-md px-2 py-0.5 rounded-lg border border-purple-300 shadow-sm">
                              <CheckCircle2 className="w-3.5 h-3.5 text-purple-700" />
                              <span>Proposal Made</span>
                            </span>
                          )}
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    ...
                    <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                      ...
                      {hasProposalMade ? (
                        <div className="inline-flex items-center space-x-1.5 text-xs font-bold text-purple-900 bg-purple-100 border border-purple-300 px-4 py-2 rounded-xl shadow-sm">
                          <CheckCircle2 className="w-4 h-4 text-purple-700" />
                          <span>Proposal already made</span>
                        </div>
                      ) : (
                        <button
            ...
            <form onSubmit={handleSubmitProposal} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Autonomous Solar-IoT Fluoride Nanofiltration & Telemetry"
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Estimated Beneficiaries / People Impacted (Lives)</label>
                <input
                  type="number"
                  min="0"
                  placeholder="e.g. 50000"
                  value={proposalForm.peopleImpacted}
                  onChange={(e) => setProposalForm({ ...proposalForm, peopleImpacted: e.target.value })}
                  className="w-full p-2.5 border border-slate-200 text-slate-900 bg-slate-50 font-medium rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 focus:outline-none"
                />
              </div>
